...
from fastapi import FastAPI, Response, Request
from fastapi.middleware.cors import CORSMiddleware
from BaseModels import LoginCredentials, Token, Equipment, Slot_Request
from hashlib import sha256
import database_handler
...
async def show_available_slots_equipment(token: Token, equipment: Equipment):
    try:
        # current_user = list(request.cookies)(0]
        current_user = token.token
        db = open_connection(current_user)
...
async def request_a_slot_for_project(token: Token, request: Slot_Request):
    try:
        # current_user = list(request.cookies)(0]
        current_user = token.token
        db = open_connection(current_user)
        result = database_handler.request_a_slot_for_project(db, request.ID)
        db = None # dereference
        return {"message":result}
    except Exception as err:
        print("error_show_user", err)
